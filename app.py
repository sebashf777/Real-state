...
/* Don't hide the whole header — stExpandSidebarButton (the arrow that
   reopens a collapsed sidebar) lives inside it. Hiding stHeader entirely
   was destroying that control, so the sidebar could never be reopened. */
...
/* Make sure the expand-sidebar arrow is always visible and clickable */
...
# ══════════════════════════════════════════════════════════════
# SIDEBAR — openable / closable, values persist either way
# ══════════════════════════════════════════════════════════════
_DEFAULTS = {
    'project_name': "Industrial Park A1",
    'asset_type': "Last Mile Warehouse",
    'price': 4_500_000,
    'total_sqft': 50_000,
    'num_units': 6,
    'rent_per_sf': 14.0,
    ...
    'capex': 250_000,
    'use_default': True,
    'loan_manual': int(4_500_000 * 0.65),
    'rate': 6.75,
    'term': 300,
    'io_period': 24,
    'opex_type': "NNN (Triple Net)",
    'opex_ratio_nnn': 5.0,
    'opex_ratio_mg': 15.0,
    'opex_ratio_fsg': 32.0,
    'tax_pct': 1.2,
...
def _persist(name):
    """Sync a widget's value (stored under 'w_<name>') into the plain,
    non-widget-bound session_state key <name> so it survives even when
    the widget itself isn't rendered on a later run (menu closed)."""
    st.session_state[name] = st.session_state['w_' + name]
    return st.session_state[name]


if not st.session_state.menu_open:
    # Menu is closed — show a small, always-visible button to bring it back.
    reopen_col, _ = st.columns([1, 6])
    with reopen_col:
        if st.button("☰ Open Menu", key="open_menu_btn", use_container_width=True):
            st.session_state.menu_open = True
            st.rerun()
...
if st.session_state.menu_open:
    with st.sidebar:
        ...
        with close_col:
            ...
            if st.button("✕", key="close_menu_btn", help="Close this menu (use the Open Menu button to bring it back)"):
                st.session_state.menu_open = False
                st.rerun()
        st.markdown("---")

        for _name in _DEFAULTS:
            st.session_state.setdefault('w_' + _name, st.session_state[_name])

        project_name = st.text_input("🏷️ Project Name", key='w_project_name')
        _persist('project_name')
        ...
        price = st.number_input("Acquisition Price ($)", step=100_000, format="%d", key='w_price')
        _persist('price')
        total_sqft = st.number_input("Total Leasable Area (SF)", step=1_000, format="%d", key='w_total_sqft')
        _persist('total_sqft')
        num_units = st.number_input("Number of Tenants / Units", step=1, min_value=1, max_value=30, key='w_num_units')
        _persist('num_units')
        rent_per_sf = st.number_input("Base Rent ($/SF/yr)", step=0.50, format="%.2f", key='w_rent_per_sf')
        _persist('rent_per_sf')
        closing = st.number_input("Closing / Transaction Costs ($)", step=5_000, format="%d", key='w_closing')
        _persist('closing')
        arv = st.number_input("Stabilized Value ($)", step=100_000, format="%d", key='w_arv')
        _persist('arv')
        capex = st.number_input("CapEx / TI Allowance ($)", step=10_000, format="%d", key='w_capex')
        _persist('capex')

        st.markdown("#### 🏦 Financing")
        use_default = st.checkbox("Use 65% LTC (CRE default)", key='w_use_default')
        _persist('use_default')
        if use_default:
            loan = price * 0.65
            st.caption(f"Loan: ${loan:,.0f}")
        else:
            loan = st.number_input("Loan Amount ($)", step=50_000, format="%d", key='w_loan_manual')
            _persist('loan_manual')
        rate = st.number_input("Interest Rate (%)", step=0.125, format="%.3f", key='w_rate')
        _persist('rate')
        term = st.selectbox("Loan Term", [300, 360, 240, 180, 120],
                            format_func=lambda x: f"{x // 12} yrs ({x} mo)", key='w_term')
        _persist('term')
        io_period = st.number_input("Interest-Only Period (months)", step=6, min_value=0, max_value=60, key='w_io_period')
        _persist('io_period')
        ...
        if opex_type == "NNN (Triple Net)":
            opex_ratio = st.slider("OpEx (landlord only, %)", 0.0, 15.0, step=0.5, key='w_opex_ratio_nnn')
            _persist('opex_ratio_nnn')
        elif opex_type == "Modified Gross":
            opex_ratio = st.slider("OpEx Ratio (%)", 5.0, 30.0, step=1.0, key='w_opex_ratio_mg')
            _persist('opex_ratio_mg')
        else:
            opex_ratio = st.slider("OpEx Ratio (%)", 15.0, 50.0, step=1.0, key='w_opex_ratio_fsg')
            _persist('opex_ratio_fsg')
        tax_pct = st.slider("Property Tax (% of price/yr)", 0.5, 3.0, step=0.1, key='w_tax_pct')
        _persist('tax_pct')
        ins_pct = st.slider("Insurance (% of price/yr)", 0.1, 1.0, step=0.1, key='w_ins_pct')
        _persist('ins_pct')
        mgmt_fee = st.slider("Mgmt/Asset Mgmt Fee (%)", 0.0, 8.0, step=0.5, key='w_mgmt_fee')
        _persist('mgmt_fee')

        st.markdown("#### 🔴 Tenant Stress Test")
        st.caption("Simulate tenant departures and re-leasing timelines")
        reabsorption = st.slider("Re-Leasing Period (months)", 3, 24, step=3, key='w_reabsorption')
        _persist('reabsorption')
        downtime_mo = st.slider("Free Rent / Downtime (months)", 0, 12, step=1, key='w_downtime_mo')
        _persist('downtime_mo')
        leasing_cost_pct = st.slider("Leasing Commission (% of annual rent)", 2.0, 8.0, step=0.5, key='w_leasing_cost_pct')
        _persist('leasing_cost_pct')

        st.markdown("#### 📈 Hold & Exit")
        hold_yrs = st.slider("Hold Period (years)", 3, 15, key='w_hold_yrs')
        _persist('hold_yrs')
        app_rate = st.slider("Annual Appreciation (%)", 0.0, 8.0, step=0.5, key='w_app_rate')
        _persist('app_rate')
        rent_escal = st.slider("Annual Rent Escalation (%)", 0.0, 5.0, step=0.25, key='w_rent_escal')
        _persist('rent_escal')
        exit_cap = st.slider("Exit Cap Rate (%)", 4.0, 9.0, step=0.25, key='w_exit_cap')
        _persist('exit_cap')

        st.markdown("---")
        st.caption("💡 CRE calculations. Not financial advice.")
else:
    # Menu is closed: pull the last-set values straight from session_state
    # so the rest of the app keeps working with whatever was configured.
    ...
    rent_per_sf = st.session_state['rent_per_sf']
    closing = st.session_state['closing']
    arv = st.session_state['arv']
    capex = st.session_state['capex']
    use_default = st.session_state['use_default']
    loan = price * 0.65 if use_default else st.session_state['loan_manual']
    rate = st.session_state['rate']
    term = st.session_state['term']
    io_period = st.session_state['io_period']
    opex_type = st.session_state['opex_type']
    if opex_type == "NNN (Triple Net)":
        opex_ratio = st.session_state['opex_ratio_nnn']
    elif opex_type == "Modified Gross":
        opex_ratio = st.session_state['opex_ratio_mg']
    else:
        opex_ratio = st.session_state['opex_ratio_fsg']
    tax_pct = st.session_state['tax_pct']
    ins_pct = st.session_state['ins_pct']
    mgmt_fee = st.session_state['mgmt_fee']
    reabsorption = st.session_state['reabsorption']
    downtime_mo = st.session_state['downtime_mo']
    leasing_cost_pct = st.session_state['leasing_cost_pct']
    hold_yrs = st.session_state['hold_yrs']
    app_rate = st.session_state['app_rate']
    rent_escal = st.session_state['rent_escal']
    exit_cap = st.session_state['exit_cap']
