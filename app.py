...
.stress-card.critical { border-left: 3px solid var(--red); }
.stress-card.warning  { border-left: 3px solid var(--amber); }
.stress-card.ok       { border-left: 3px solid var(--green); }
...
def build_amortization(principal, annual_rate_pct, term_months):
    ...
    for month in range(1, int(term_months) + 1):
        ...
        cumint      += interest
        records.append({'Month': month, 'Payment': round(payment,2),
...
with st.sidebar:
    ...
    loan = price * 0.65 if use_default else st.number_input("Loan Amount ($)", value=int(price*0.65), step=50_000, format="%d")
    if use_default: st.caption(f"Loan: ${loan:,.0f}")
...
with col_h1:
    ...
    st.markdown(f'<div class="page-sub">{asset_icon} {asset_type} · {project_name} · {total_sqft:,} SF · {num_units} Tenants</div>',
                unsafe_allow_html=True)
with col_h2:
    ...
    st.markdown(f'<div style="text-align:right;font-size:11px;color:#5A7090;margin-top:5px;font-family:IBM Plex Mono,monospace">'
                f'Cap: {r["cap_rate"]:.2f}% · YoC: {yield_on_cost:.2f}% · DSCR: {r["dscr"]:.2f}</div>',
                unsafe_allow_html=True)

# ══════════════════════════════════════════════════════════════
# KPI ROW 1 — CORE CRE METRICS
# ══════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════
# KPI ROW 2 — PER-SF & COMMERCIAL METRICS
# ══════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════
with tab1:
    st.markdown('<div class="section-title">💵 <span>Pro Forma Income Statement</span></div>', unsafe_allow_html=True)
...
with tab2:
    ...
    for idx, (sc_name, col_obj) in enumerate(zip(scenario_names, summary_cols)):
        ...
        with col_obj:
            ...
            <div style="background:var(--card);border:1px solid var(--border);
              ...
              <div style="font-size:10px;color:#5A7090;text-transform:uppercase;letter-spacing:1px;font-family:'IBM Plex Mono',monospace;margin-bottom:8px">
                {sc_name}
              </div>
    ...
    for sc_name, sc_color in zip(scenario_names, colors_sc):
        ...
        fig_cum.add_trace(go.Scatter(
            ...
            name=sc_name, line=dict(color=sc_color, width=2),
            fill='tozeroy', fillcolor=sc_color.replace('#','rgba(').replace(',', '') + '10)'
        ))
    ...
    with col1:
        ...
        for v in vacancy_range:
            ...
            for r_sf in rent_sf_range:
                ...
                rr = full_analysis(price, r_mo, closing, loan, rate, term,
                                   v, opex_total, tax_pct, ins_pct)
    ...
    with col2:
        ...
        for occ in occ_range:
            ...
            for rt in rate_range2:
                vac = 100 - occ
                rr  = full_analysis(price, rent_monthly, closing, loan, rt, term,
                                    vac, opex_total, tax_pct, ins_pct)
...
with tab4:
    ...
    with col1:
        ...
        if io_period > 0:
            fig_am.add_vrect(x0=0, x1=io_period, fillcolor='rgba(245,158,11,0.08)',
                             ...
                             annotation_text=f"I/O Period", annotation_position="top left",
                             annotation_font_color=C['amber'])
    ...
    c4.metric("Equity Yr 5", f"${am[am['Month']==min(60,len(am))]['Equity'].values[-1]:,.0f}" if len(am)>=60 else "N/A")
...
# ════════════════════════════════════════════════════════════
with tab5:
    st.markdown('<div class="section-title">📈 <span>Hold Period Projections & Exit</span></div>', unsafe_allow_html=True)
    ...
    for y in years:
        ...
        eq      = prop_v - bal
        rr      = full_analysis(price, r_mo_y, closing, loan, rate, term,
                                vacancy_pct, opex_total, tax_pct, ins_pct)
...
with tab6:
    ...
    with col1:
        checks = [
            ...
            (f"Stress Test: Anchor Loss Positive CF",
             float(stress_df[stress_df['Scenario']=='⚡ Anchor Loss (25%)']['Cash Flow'].min()) > -r['payment']*0.5,
    ...
    with col2:
        ...
        <div style="background:var(--card);border:1px solid var(--border);border-radius:12px;
          ...
          <div style="font-size:10px;color:#5A7090;text-transform:uppercase;letter-spacing:2px;font-family:'IBM Plex Mono',monospace">
            Overall Deal Score
          </div>
          <div style="font-family:'Syne',sans-serif;font-size:56px;font-weight:800;color:{score_col};line-height:1.1;margin:8px 0">
            {overall}
          </div>
...
with tab7:
    ...
    report_rows = [
        ...
        ('','',''),
        ('── STRESS TEST ──','',''),
        ('Anchor Loss — Worst Month CF', f'${float(stress_df[stress_df["Scenario"]=="⚡ Anchor Loss (25%)"]["Cash Flow"].min()):,.0f}', ''),
        ('Cascade Vacancy — Worst Month CF', f'${float(stress_df[stress_df["Scenario"]=="⚠️ Cascade Vacancy (40%)"]["Cash Flow"].min()):,.0f}', ''),
