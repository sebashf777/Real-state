...
# ══════════════════════════════════════════════════════════════
# SIDEBAR — inputs live inside a native Streamlit expander.
# Clicking the header collapses/expands it, any number of times.
# Because the widgets stay mounted even while collapsed, nothing
# ever resets — this replaces the earlier custom toggle, which
# relied on manually tracking open/closed state and broke because
# Streamlit clears a widget's stored value whenever that widget
# isn't rendered during a run.
# ══════════════════════════════════════════════════════════════
with st.sidebar:
    st.markdown("""
    <div style='padding:16px 0 8px'>
      <div style='font-family:"Syne",sans-serif;font-size:26px;font-weight:800;
                  ...
                  letter-spacing:-1px'>Property Calculator</div>
    ...
    with st.expander("⚙️ Deal Inputs", expanded=True):
...
with tab7:
    ...
    st.download_button("⬇️ Download Full Report (CSV)", data=csv,
                       file_name=f"{project_name.replace(' ', '_')}_Property_Calculator.csv", mime='text/csv')

# ── FOOTER ────────────────────────────────────────────────────
st.markdown(f"""
<div style='text-align:center;padding:40px 0 20px;color:#243045;font-size:11px;
            font-family:"IBM Plex Mono",monospace'>
  Property Analyzer Calculator · CRE Investment Analyzer · Not financial advice · {asset_type}
</div>
""", unsafe_allow_html=True)
